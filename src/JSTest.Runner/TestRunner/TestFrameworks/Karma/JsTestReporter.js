...
/*
        ...
        outcome = result.skipped ? TestOutcome.None : (result.success ? TestOutcome.Passed : TestOutcome.Failed);

        let specName = result.suite.slice();
        specName.push(result.description);
        specName = specName.join(':');
        //'::' + result.fullName + '::' + testFilePath

        if (!result.success) {
           // const errorMessage = formatError(result.log[0]);
        }
        ...
        if (this.discovery) {
            this.callbacks.handleSpecFound(result.fullName,
                                                   unitTestName,
                                                   this.configFilePath,
                                                   undefined,
                                                   specName,
            ...
            this.callbacks.handleSpecResult(codeBase,
                                                    unitTestName,
                                                    this.configFilePath,
                                                    outcome,
                                                    undefined,
